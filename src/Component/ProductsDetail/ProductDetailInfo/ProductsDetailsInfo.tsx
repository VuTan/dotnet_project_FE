...
const ProductsDetailsInfo: React.FC<ProductsDetailsInfoProps> = ({ productId }) => {
    ...
    const { name, price, sizes, toppings } = product;

    return (
        <div className="info_container">
            <h1 className="title_info">{name}</h1>
                    ...
                    <div className="section_2">
                        <div className="option_sizes">
                            ...
                            <div className="product_options">
                                <div id="ax_1055578447" className="opt_size">
                                    {sizes.map(size => (
                                        <div
                                            key={size.value}
                                            className={`product__info__item__list__item ${selectedSize === size.value ? 'selected' : ''}`}
                                        ...
                                        >
                                            ...
                                            <input type="radio" name="size" value={size.value} hidden/>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="product_options">
                                {toppings.map(option => (
                                    <label key={option.value}
                                           className={`option_item ${selectedToppings.includes(option.value) ? 'selected' : ''}`}>
                                        <input
                                            type="checkbox"
                                            className="checkbox"
                                            id={option.value}
